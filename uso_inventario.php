...
<img src="images/logocd.png" alt="" class="img-responsive">
...
<body>
  ...
  if ( $action == 'tomar_producto' ){
    echo "<div class='alert alert-success' > Producto tomado del inventario exitosamente.</div>";
  }
  ...
  if ($num > 0){
    ...
    while ( $row = $stmt -> fetch ( PDO::FETCH_ASSOC)){
      ...
      echo "<td>";
        ...
        echo "<a href ='#' onclick='tomar_producto( {$idproducto} );'><span class='btn btn-warning glyphicon-minus '> Tomar Producto</span></a>" ;
        echo "<a href ='#' onclick='comprar_producto( {$idproducto} );'><span class='btn btn-default glyphicon-plus '> Agregar Producto</span></a>" ;
      echo "</td>";
      echo "</tr>";
    }
    //end table
  ...
  function tomar_producto( idproducto ){
    var answer = confirm ('Esta Seguro que desea tomar el producto?')
    if ( answer ){
      //if user clicked ok, pass id to delete.php and execute the delete query
      window.location = 'tomar_producto.php?idproducto='  + idproducto;
    }
  }
  </script>

  <script type='text/javascript'>
  function comprar_producto( idproducto ){
    var answer = confirm ('Esta Seguro que desea agregar el producto?')
    if ( answer ){
      //if user clicked ok, pass id to delete.php and execute the delete query
      window.location = 'comprar_producto.php?idproducto='  + idproducto;
    }
  }
  </script>
